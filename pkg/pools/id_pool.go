...
func (pool *IDPool) Put(id uint32) {
	...
	if id < 1 || id > pool.maxUsed {
		log.Panic().Msgf("IDPool.Put(%v): invalid value, must be in the range [1,%v]", id, pool.maxUsed)
	}

	if pool.used[id] {
		log.Panic().Msgf("IDPool.Put(%v): can't put value that was already recycled", id)
	}
